...
- (id)initWithAppKey:(NSString *)appKey appSecret:(NSString *)appSecret text:(NSString *)text image:(UIImage *)image
{
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, 480)])
    {
        ...
        atFriends = [UIButton buttonWithType:UIButtonTypeCustom];
		[atFriends setShowsTouchWhenHighlighted:YES];
		[atFriends setFrame:CGRectMake(15, 180, 50, 30)];
		//[atFriends setContentMode:UIViewContentModeCenter];
        [atFriends.titleLabel setFont:[UIFont boldSystemFontOfSize:13.0f]];
        [atFriends setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        [atFriends setTitle:NSLocalizedString(@"@好友", nil) forState:UIControlStateNormal];
 		//[atFriends setImage:[UIImage imageNamed:@"delete.png"] forState:UIControlStateNormal];
		[atFriends addTarget:self action:@selector(onatFriendsTouched:) forControlEvents:UIControlEventTouchUpInside];
		[panelView addSubview:atFriends];
...
- (void)onSendButtonTouched:(id)sender
{
    [delegate sendViewDidClickSendButton:self];
...
-(void)onatFriendsTouched:(id)sender
{
    [delegate sendViewDidClickAtButton:self];//at功能
}
...
- (void)keyboardWillShow:(NSNotification*)notification
{
    ...
    {
		//if (panelView.frame.origin.y>51) {
