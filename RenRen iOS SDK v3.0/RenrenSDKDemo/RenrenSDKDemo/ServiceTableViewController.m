...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.sectionHeaders = [NSArray arrayWithObjects:@"用户信息", @"好友", @"相册", @"发布内容", @"一键发布", @"Widget Dialog",@"支付", nil];
        self.cellText = [NSArray arrayWithObjects:[NSArray arrayWithObjects:@"当前用户信息", @"指定用户信息", nil], [NSArray arrayWithObjects:@"好友ID列表",@"好友信息列表",nil], [NSArray arrayWithObjects:@"相册列表", @"新建相册", nil], [NSArray arrayWithObjects:@"上传照片", @"发布状态", nil], [NSArray arrayWithObject:@"一键上传照片"], [NSArray arrayWithObjects:@"Feed Dialog－导航页面", @"Like Dialog－弹层页面", nil],[NSArray arrayWithObjects:@"支付", @"查询订单-弹层页面", @"删除订单", nil],nil];
        self.cellDetailText = [NSArray arrayWithObjects:[NSArray arrayWithObjects:@"getUsersInfo",@"getUsersInfo", nil], [NSArray arrayWithObjects:@"getFriends",@"getFriendsInfo", nil], [NSArray arrayWithObjects:@"getAlbums", @"createAlbum", nil], [NSArray arrayWithObjects:@"publishPhoto", @"postStatus", nil], [NSArray arrayWithObjects:@"publishPhotoSimply", nil], [NSArray arrayWithObjects:@"feedDialog", @"likeDialog", nil],[NSArray arrayWithObjects:@"payOrder", @"queryOrderList", @"deleteOrderList", nil], nil];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    if ([self.renren isSessionValid]) {
        ...
        self.renrenPay  = [self.renren getRenrenPayWithSecret:@"YOUR APP SECRET" andLocalMem:YES];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return [(NSArray*)[self.cellText objectAtIndex:section] count];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            PayViewController *payViewController = [[PayViewController alloc] initWithNibName:nil bundle:[NSBundle mainBundle]];
            payViewController.renren = self.renren;
            payViewController.renrenPay = self.renrenPay;
